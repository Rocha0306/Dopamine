...
public class Navegacao {
    ...
    public void VerificarUsuario() throws SQLException {
        Conexao ConexaoBD = new Conexao();
        FuncionalidadesCalculadoraDeDopamina App = new FuncionalidadesCalculadoraDeDopamina();
        Dados ArmazenamentoDeDados = new Dados();
        ClasseDeVerificacao Verificar = new ClasseDeVerificacao();
        if(VerifyCadastro == true) {
            ConexaoBD.LoginUsuarioNoDB();
            Verificar.VerificadorDeLogin();


        }

        else {
            ConexaoBD.CadastroUsuarioNoDB();
            App.MensagensAposCadastro();
            App.FuncionalidadeDoAplicativo();
